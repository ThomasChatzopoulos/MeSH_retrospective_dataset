...
def restrictionsAnalysis(path, first_year, last_year):
    """
    ...
    for year in range(first_year, last_year + 1):
        counts = (
            pd.DataFrame(
                pd.DataFrame(
                    pd.read_csv(path + f"/newDescriptorsFull_{year}.csv")["Rest. Level"].value_counts()
                ).T,
    ...
    merged_results.at["Total", "0: No (old concept) subdivision"] = merged_results["0: No (old concept) subdivision"].sum()
    ...
    merged_results.at["Total", "4: right article num"] = merged_results["4: right article num"].sum()

    merged_results.to_csv(f"{path}/restrictions_nalysis.csv")
